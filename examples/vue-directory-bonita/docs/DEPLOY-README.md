# Deploy / Déployer / Desplegar — `page-appDirectoryBonitaVueHome.zip`
...
- `page-appDirectoryBonitaVueHome.zip` — the Bonita custom page (a Vue 3 SPA)
...
Click **+ Add a resource**, select `page-appDirectoryBonitaVueHome.zip`, confirm.
The page now appears in the list with displayName `Directory Bonita Vue Home`.
...
| Field | Value |
|-------|-------|
| Token | `appDirectoryBonitaVue` |
| Display name | `Directory Bonita Vue` |
...
1. Click **+ Add page**
2. Select `Directory Bonita Vue Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaVue/home/?_l=en
```

If everything is correct, the Vue 3 SPA loads, auto-detects your Bonita session, and shows the task list. **Listo.**
...
| Page imports OK but `/bonita/apps/appDirectoryBonitaVue/home/` 404s | Check (a) Application token spelling, (b) Page token = `home`, (c) Your user has the application's profile. |
...
- `page-appDirectoryBonitaVueHome.zip` — la custom page Bonita (SPA Vue 3)
...
Cliquez sur **+ Ajouter une ressource**, sélectionnez `page-appDirectoryBonitaVueHome.zip`, validez.
La page apparaît avec le nom `Directory Bonita Vue Home`.
...
2. Sélectionnez `Directory Bonita Vue Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaVue/home/?_l=fr
```

Si tout est correct, la SPA Vue 3 se charge, détecte automatiquement la session Bonita et affiche la liste des tâches. **C'est terminé.**
...
| Page importée mais `/bonita/apps/appDirectoryBonitaVue/home/` 404 | Vérifiez (a) l'orthographe du token application, (b) le token de la page = `home`, (c) votre utilisateur a bien le profil de l'application. |
...
- `page-appDirectoryBonitaVueHome.zip` — la custom page de Bonita (SPA Vue 3)
...
Haz clic en **+ Añadir un recurso**, selecciona `page-appDirectoryBonitaVueHome.zip`, confirma.
La página aparece en la lista con el nombre `Directory Bonita Vue Home`.
...
|-------|-------|
| Token | `appDirectoryBonitaVue` |
| Nombre visible | `Directory Bonita Vue` |
...
2. Selecciona `Directory Bonita Vue Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaVue/home/?_l=es
```

Si todo está bien, la SPA Vue 3 se carga, detecta automáticamente la sesión de Bonita y muestra la lista de tareas. **Listo.**
...
| Página importada pero `/bonita/apps/appDirectoryBonitaVue/home/` 404 | Comprueba (a) el token de la aplicación, (b) que el token de la página sea `home`, (c) que tu usuario tenga el perfil de la aplicación. |
