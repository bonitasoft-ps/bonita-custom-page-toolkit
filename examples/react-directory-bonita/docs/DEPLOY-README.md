# Deploy / Déployer / Desplegar — `page-appDirectoryBonitaReactHome.zip`
...
- `page-appDirectoryBonitaReactHome.zip` — the Bonita custom page (a React SPA)
...
Click **+ Add a resource**, select `page-appDirectoryBonitaReactHome.zip`, confirm.
The page now appears in the list with displayName `Directory Bonita React Home`.
...
| Field | Value |
|-------|-------|
| Token | `appDirectoryBonitaReact` |
| Display name | `Directory Bonita React` |
...
1. Click **+ Add page**
2. Select `Directory Bonita React Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaReact/home/?_l=en
```

If everything is correct, the React SPA loads, auto-detects your Bonita session, and shows the task list. **Listo.**
...
| Page imports OK but `/bonita/apps/appDirectoryBonitaReact/home/` 404s | Check (a) Application token spelling, (b) Page token = `home`, (c) Your user has the application's profile. |
...
- `page-appDirectoryBonitaReactHome.zip` — la custom page Bonita (SPA React)
...
Cliquez sur **+ Ajouter une ressource**, sélectionnez `page-appDirectoryBonitaReactHome.zip`, validez.
La page apparaît avec le nom `Directory Bonita React Home`.
...
2. Sélectionnez `Directory Bonita React Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaReact/home/?_l=fr
```

Si tout est correct, la SPA React se charge, détecte automatiquement la session Bonita et affiche la liste des tâches. **C'est terminé.**
...
| Page importée mais `/bonita/apps/appDirectoryBonitaReact/home/` 404 | Vérifiez (a) l'orthographe du token application, (b) le token de la page = `home`, (c) votre utilisateur a bien le profil de l'application. |
...
- `page-appDirectoryBonitaReactHome.zip` — la custom page de Bonita (SPA React)
...
Haz clic en **+ Añadir un recurso**, selecciona `page-appDirectoryBonitaReactHome.zip`, confirma.
La página aparece en la lista con el nombre `Directory Bonita React Home`.
...
|-------|-------|
| Token | `appDirectoryBonitaReact` |
| Nombre visible | `Directory Bonita React` |
...
2. Selecciona `Directory Bonita React Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaReact/home/?_l=es
```

Si todo está bien, la SPA React se carga, detecta automáticamente la sesión de Bonita y muestra la lista de tareas. **Listo.**
...
| Página importada pero `/bonita/apps/appDirectoryBonitaReact/home/` 404 | Comprueba (a) el token de la aplicación, (b) que el token de la página sea `home`, (c) que tu usuario tenga el perfil de la aplicación. |
