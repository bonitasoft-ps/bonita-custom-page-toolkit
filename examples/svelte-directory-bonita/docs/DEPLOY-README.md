# Deploy / Déployer / Desplegar — `page-appDirectoryBonitaSvelteHome.zip`
...
- `page-appDirectoryBonitaSvelteHome.zip` — the Bonita custom page (a Svelte 5 SPA)
...
Click **+ Add a resource**, select `page-appDirectoryBonitaSvelteHome.zip`, confirm.
The page now appears in the list with displayName `Directory Bonita Svelte Home`.
...
| Field | Value |
|-------|-------|
| Token | `appDirectoryBonitaSvelte` |
| Display name | `Directory Bonita Svelte` |
...
1. Click **+ Add page**
2. Select `Directory Bonita Svelte Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaSvelte/home/?_l=en
```

If everything is correct, the Svelte 5 SPA loads, auto-detects your Bonita session, and shows the task list. **Listo.**
...
| Page imports OK but `/bonita/apps/appDirectoryBonitaSvelte/home/` 404s | Check (a) Application token spelling, (b) Page token = `home`, (c) Your user has the application's profile. |
...
- `page-appDirectoryBonitaSvelteHome.zip` — la custom page Bonita (SPA Svelte 5)
...
Cliquez sur **+ Ajouter une ressource**, sélectionnez `page-appDirectoryBonitaSvelteHome.zip`, validez.
La page apparaît avec le nom `Directory Bonita Svelte Home`.
...
2. Sélectionnez `Directory Bonita Svelte Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaSvelte/home/?_l=fr
```

Si tout est correct, la SPA Svelte 5 se charge, détecte automatiquement la session Bonita et affiche la liste des tâches. **C'est terminé.**
...
| Page importée mais `/bonita/apps/appDirectoryBonitaSvelte/home/` 404 | Vérifiez (a) l'orthographe du token application, (b) le token de la page = `home`, (c) votre utilisateur a bien le profil de l'application. |
...
- `page-appDirectoryBonitaSvelteHome.zip` — la custom page de Bonita (SPA Svelte 5)
...
Haz clic en **+ Añadir un recurso**, selecciona `page-appDirectoryBonitaSvelteHome.zip`, confirma.
La página aparece en la lista con el nombre `Directory Bonita Svelte Home`.
...
|-------|-------|
| Token | `appDirectoryBonitaSvelte` |
| Nombre visible | `Directory Bonita Svelte` |
...
2. Selecciona `Directory Bonita Svelte Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaSvelte/home/?_l=es
```

Si todo está bien, la SPA Svelte 5 se carga, detecta automáticamente la sesión de Bonita y muestra la lista de tareas. **Listo.**
...
| Página importada pero `/bonita/apps/appDirectoryBonitaSvelte/home/` 404 | Comprueba (a) el token de la aplicación, (b) que el token de la página sea `home`, (c) que tu usuario tenga el perfil de la aplicación. |
