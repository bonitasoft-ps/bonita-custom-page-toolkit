# Deploy / Déployer / Desplegar — `page-appDirectoryBonitaSolidHome.zip`
...
- `page-appDirectoryBonitaSolidHome.zip` — the Bonita custom page (a SolidJS SPA)
...
Click **+ Add a resource**, select `page-appDirectoryBonitaSolidHome.zip`, confirm.
The page now appears in the list with displayName `Directory Bonita Solid Home`.
...
| Field | Value |
|-------|-------|
| Token | `appDirectoryBonitaSolid` |
| Display name | `Directory Bonita Solid` |
...
1. Click **+ Add page**
2. Select `Directory Bonita Solid Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaSolid/home/?_l=en
```

If everything is correct, the SolidJS SPA loads, auto-detects your Bonita session, and shows the task list. **Listo.**
...
| Page imports OK but `/bonita/apps/appDirectoryBonitaSolid/home/` 404s | Check (a) Application token spelling, (b) Page token = `home`, (c) Your user has the application's profile. |
...
- `page-appDirectoryBonitaSolidHome.zip` — la custom page Bonita (SPA SolidJS)
...
Cliquez sur **+ Ajouter une ressource**, sélectionnez `page-appDirectoryBonitaSolidHome.zip`, validez.
La page apparaît avec le nom `Directory Bonita Solid Home`.
...
2. Sélectionnez `Directory Bonita Solid Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaSolid/home/?_l=fr
```

Si tout est correct, la SPA SolidJS se charge, détecte automatiquement la session Bonita et affiche la liste des tâches. **C'est terminé.**
...
| Page importée mais `/bonita/apps/appDirectoryBonitaSolid/home/` 404 | Vérifiez (a) l'orthographe du token application, (b) le token de la page = `home`, (c) votre utilisateur a bien le profil de l'application. |
...
- `page-appDirectoryBonitaSolidHome.zip` — la custom page de Bonita (SPA SolidJS)
...
Haz clic en **+ Añadir un recurso**, selecciona `page-appDirectoryBonitaSolidHome.zip`, confirma.
La página aparece en la lista con el nombre `Directory Bonita Solid Home`.
...
|-------|-------|
| Token | `appDirectoryBonitaSolid` |
| Nombre visible | `Directory Bonita Solid` |
...
2. Selecciona `Directory Bonita Solid Home`
...
```
http://localhost:8080/bonita/apps/appDirectoryBonitaSolid/home/?_l=es
```

Si todo está bien, la SPA SolidJS se carga, detecta automáticamente la sesión de Bonita y muestra la lista de tareas. **Listo.**
...
| Página importada pero `/bonita/apps/appDirectoryBonitaSolid/home/` 404 | Comprueba (a) el token de la aplicación, (b) que el token de la página sea `home`, (c) que tu usuario tenga el perfil de la aplicación. |
