# Deploy / Déployer / Desplegar — `page-__NAME__.zip`
...
- `page-__NAME__.zip` — the Bonita custom page (a __FRAMEWORK_LABEL__ SPA)
...
Click **+ Add a resource**, select `page-__NAME__.zip`, confirm.
The page now appears in the list with displayName `__DISPLAY_NAME__`.
...
```
http://localhost:8080/bonita/apps/__APP_TOKEN__/__PAGE_TOKEN__/?_l=en
```

If everything is correct, the __FRAMEWORK_LABEL__ SPA loads, auto-detects your Bonita session, and shows the task list. **Listo.**
...
| Page imports OK but `/bonita/apps/__APP_TOKEN__/home/` 404s | Check (a) Application token spelling, (b) Page token = `home`, (c) Your user has the application's profile. |
...
- `page-__NAME__.zip` — la custom page Bonita (SPA __FRAMEWORK_LABEL__)
...
Cliquez sur **+ Ajouter une ressource**, sélectionnez `page-__NAME__.zip`, validez.
La page apparaît avec le nom `__DISPLAY_NAME__`.
...
2. Sélectionnez `__DISPLAY_NAME__`
...
```
http://localhost:8080/bonita/apps/__APP_TOKEN__/__PAGE_TOKEN__/?_l=fr
```

Si tout est correct, la SPA __FRAMEWORK_LABEL__ se charge, détecte automatiquement la session Bonita et affiche la liste des tâches. **C'est terminé.**
...
| Page importée mais `/bonita/apps/__APP_TOKEN__/home/` 404 | Vérifiez (a) l'orthographe du token application, (b) le token de la page = `home`, (c) votre utilisateur a bien le profil de l'application. |
...
- `page-__NAME__.zip` — la custom page de Bonita (SPA __FRAMEWORK_LABEL__)
...
Haz clic en **+ Añadir un recurso**, selecciona `page-__NAME__.zip`, confirma.
La página aparece en la lista con el nombre `__DISPLAY_NAME__`.
...
|-------|-------|
| Token | `__APP_TOKEN__` |
| Nombre visible | `__DISPLAY_NAME__` |
...
```
http://localhost:8080/bonita/apps/__APP_TOKEN__/__PAGE_TOKEN__/?_l=es
```

Si todo está bien, la SPA __FRAMEWORK_LABEL__ se carga, detecta automáticamente la sesión de Bonita y muestra la lista de tareas. **Listo.**
...
| Página importada pero `/bonita/apps/__APP_TOKEN__/home/` 404 | Comprueba (a) el token de la aplicación, (b) que el token de la página sea `home`, (c) que tu usuario tenga el perfil de la aplicación. |
